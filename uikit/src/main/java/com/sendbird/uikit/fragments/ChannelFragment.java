...
public class ChannelFragment extends BaseMessageListFragment<MessageListAdapter, MessageListComponent, ChannelModule, ChannelViewModel> {
    ...
    private void onInputRightButtonClicked(@NonNull View view) {
        ...
        if (inputText != null && !TextUtils.isEmpty(inputText.getText())) {
            ...
            // Check if replying to admin message
            if (targetMessage != null && targetMessage instanceof AdminMessage) {
                // For admin messages, send as normal message with admin context
                String adminReplyMessage = createAdminReplyMessage(targetMessage.getMessage(), editableText.toString());
                params = new UserMessageCreateParams(adminReplyMessage);
                
                // Add custom data to indicate this is an admin reply
                params.setData("admin_reply");
                params.setCustomType("admin_response");
                
                Logger.d("Sending normal message in response to admin message");
            } else if (targetMessage != null && channelConfig.getReplyType() != ReplyType.NONE) {
                // Normal user message reply with threading
    ...
    protected List<DialogListItem> makeMessageContextMenu(@NonNull BaseMessage message) {
        ...
        // For admin messages, show "Reply" instead of "Reply in thread"
        int replyStringRes = R.string.sb_text_channel_anchor_reply;
        int replyDrawableRes = R.drawable.icon_reply;
        
        if (!(message instanceof AdminMessage)) {
            // For non-admin messages, use the configured reply type
            replyStringRes = channelConfig.getReplyType() == ReplyType.THREAD ? R.string.sb_text_channel_anchor_reply_in_thread : R.string.sb_text_channel_anchor_reply;
            replyDrawableRes = channelConfig.getReplyType() == ReplyType.THREAD ? R.drawable.icon_thread : R.drawable.icon_reply;
        }
    ...
    /**
     * Creates a formatted message when replying to an admin message
     *
     * @param adminMessage The original admin message content
     * @param userReply The user's reply content
     * @return Formatted message string
     */
    private String createAdminReplyMessage(String adminMessage, String userReply) {
        // Format: "Re: [Admin] Original message" + "\n" + "User reply"
        StringBuilder formattedMessage = new StringBuilder();
        
        // Add admin message context
        formattedMessage.append(getContext().getString(R.string.reply_to));
        
        // Truncate admin message if too long
        if (adminMessage != null && adminMessage.length() > 50) {
            formattedMessage.append(adminMessage.substring(0, 50)).append("...");
        } else if (adminMessage != null) {
            formattedMessage.append("("+adminMessage+")");
        }
        
        // Add separator and user reply
