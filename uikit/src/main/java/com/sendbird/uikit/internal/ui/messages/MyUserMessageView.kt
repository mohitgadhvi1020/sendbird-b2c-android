...
import com.sendbird.uikit.test.databinding.SbViewMyUserMessageComponentBinding
...
internal class MyUserMessageView @JvmOverloads internal constructor(
    ...
    val defStyle: Int = R.attr.sb_widget_my_user_message
    ...
    override fun drawMessage(channel: GroupChannel, message: BaseMessage, params: MessageListUIParams) {
        ...
        // Custom styling for admin reply messages
        // Check both message content and custom data for admin replies
        val isAdminReply = message.message.startsWith(context.getString(R.string.reply_to)) ||
                          message.data == "admin_reply" || 
                          message.customType == "admin_response"
        
        if (isAdminReply) {
            // Change background for admin reply messages (using drawable with border)
            binding.contentPanel.setBackgroundResource(R.drawable.sb_shape_chat_bubble_admin_reply)

            binding.tvMessage.setTextColor(ContextCompat.getColor(context, R.color.background_500))
            /*messageUIConfig?.let {
                it.myMessageTextUIConfig.textColor =
                    ContextCompat.getColor(context, R.color.background_700)
            }*/
            // Optional: Add some visual indicator or different styling
            binding.tvMessage.alpha = 0.95f
        } else {

            binding.contentPanel.setBackgroundResource(R.drawable.sb_shape_chat_bubble_me)

            binding.tvMessage.setTextColor(ContextCompat.getColor(context, R.color.background_50))
            /*messageUIConfig?.let {
                it.myMessageTextUIConfig.textColor =
                    ContextCompat.getColor(context, R.color.background_700)
            }*/
            // Optional: Add some visual indicator or different styling
         /*   binding.tvMessage.alpha = 0.95f
            // Reset to default styling for normal messages
            messageUIConfig?.let {
                it.myMessageBackground?.let { background -> binding.contentPanel.background = background }
            } ?: run {
                // Fallback to original background if no custom config
                val messageBackground = R.drawable.sb_shape_chat_bubble
                val messageBackgroundTint = context.theme.obtainStyledAttributes(
                    attrs, R.styleable.MessageView_User, defStyle, 0
                ).getColorStateList(R.styleable.MessageView_User_sb_message_me_background_tint)
                binding.contentPanel.background = DrawableUtils.setTintList(context, messageBackground, messageBackgroundTint)
            }
            
            // Reset text color to default
            binding.tvMessage.setTextColor(ContextCompat.getColor(context, R.color.ondark_text_high_emphasis))
